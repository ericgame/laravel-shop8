...
// 引入 lodash，lodash 是一個實用工具庫，提供了很多常用的方法
...
Vue.component('select-district', {
  ...
  watch: {
    ...
    // 當選擇的區發生改變時觸發
    districtId() {
      // 觸發一個名為 change 的 Vue 事件，事件的值就是當前選中的省市區名稱，格式為數組
  ...
  methods: {
    ...
    setFromValue(value) {
      ...
      // 如果數組長度為0，則將省清空（由於我們定義了觀察器，會聯動觸發將城市和地區清空）
      if (value.length === 0) {
        this.provinceId = '';
        return;
      }
      // 從當前省列表中找到與數組第一個元素同名的項的索引
      ...
      // 由於觀察器的作用，這個時候城市列表已經變成了對應省的城市列表
      // 從當前城市列表找到與數組第二個元素同名的項的索引
